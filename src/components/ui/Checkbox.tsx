...
const CheckBoxVariants = ({ variant = "circle" }: CheckboxProps) => {
  const baseStyles = "peer h-6 w-6 shrink-0 border-2 border-primary ring-offset-background focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 disabled:cursor-not-allowed disabled:opacity-50 data-[state=checked]:bg-primary data-[state=checked]:text-primary-foreground transition-all duration-300 ease-in-out hover:scale-105"
  const variantStyles = {
    circle: "rounded-full",
    square: "rounded-md",
  }
